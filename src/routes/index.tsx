                  ...
                  >
                    <div className={"flex h-full flex-col justify-between p-2 " + (qty > 0 ? "pb-8" : "")}>
                      ...
                      <div className="flex items-end justify-between text-[10px]">
                        ...
                        <span className={out ? "opacity-70 text-destructive" : "opacity-70 font-medium"}>
  ...
  return (
    <>
      <SheetContent className="flex w-full flex-col sm:max-w-md">
        ...
        {items.length > 0 && (
          <div className="space-y-3 border-t pt-4">
            ...
            <p className="text-center text-xs text-muted-foreground">Informe seu nome para registrar o pedido antes de enviar pelo WhatsApp.</p>
          </div>
        )}
      </SheetContent>
      ...
      <CheckoutDialog
        ...
        onSuccess={() => { setShowCheckout(false); onClear(); onDone(); }}
      />
    </>
  );
}
...
function CheckoutDialog({
  open, items, totalCents, onClose, onSuccess,
}: {
  open: boolean;
  items: { sticker: Sticker; team?: Team; quantity: number }[];
  totalCents: number;
  ...
  async function handleSubmit() {
    ...
    if (!contact.trim()) { toast.error("Informe seu WhatsApp."); return; }
    setBusy(true);
    try {
      const { data: order, error: orderErr } = await supabase
        .from("orders")
        .insert({ customer_name: name.trim(), customer_contact: contact.trim(), note: note.trim() || null, total_cents: totalCents, status: "pending" })
        .select().single();
      if (orderErr) throw orderErr;
      const { error: itemsErr } = await supabase.from("order_items").insert(
        items.map((i) => ({ order_id: order.id, sticker_id: i.sticker.id, quantity: i.quantity, unit_price_cents: i.sticker.price_cents }))
      );
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Preencha seus dados para registrar o orçamento.</DialogDescription>
        </DialogHeader>
        <div className="space-y-3 pt-2">
          <div>
            <Label htmlFor="co-name">Seu nome *</Label>
            <Input id="co-name" placeholder="Ex: Maria Silva" value={name} onChange={(e) => setName(e.target.value)} disabled={busy} />
          </div>
          <div>
            <Label htmlFor="co-contact">Seu WhatsApp *</Label>
            <Input id="co-contact" placeholder="Ex: (31) 99999-0000" value={contact} onChange={(e) => setContact(e.target.value)} disabled={busy} />
          </div>
          <div>
            <Label htmlFor="co-note">Observação (opcional)</Label>
            <Textarea id="co-note" placeholder="Ex: Retirar na quinta-feira" value={note} onChange={(e) => setNote(e.target.value)} disabled={busy} className="min-h-[70px]" />
          </div>
          ...
          <Button onClick={handleSubmit} disabled={busy} className="w-full" size="lg">
            {busy ? "Registrando..." : <><Send className="mr-2 h-4 w-4" />Registrar e enviar pelo WhatsApp</>}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
